...
const deleteFeedFollowByFUrlUser = `-- name: DeleteFeedFollowByFUrlUser :exec
...
type DeleteFeedFollowByFUrlUserParams struct {
	Name string
	Url  string
}

func (q *Queries) DeleteFeedFollowByFUrlUser(ctx context.Context, arg DeleteFeedFollowByFUrlUserParams) error {
	_, err := q.db.ExecContext(ctx, deleteFeedFollowByFUrlUser, arg.Name, arg.Url)
	return err
}
...
const fetchFeedId = `-- name: FetchFeedId :one
SELECT id FROM feeds where url = $1
`

func (q *Queries) FetchFeedId(ctx context.Context, url string) (uuid.UUID, error) {
	row := q.db.QueryRowContext(ctx, fetchFeedId, url)
	var id uuid.UUID
	err := row.Scan(&id)
	return id, err
}
...
func (q *Queries) MarkFeedFetched(ctx context.Context, arg MarkFeedFetchedParams) error {
	_, err := q.db.ExecContext(ctx, markFeedFetched, arg.LastFetchedAt, arg.UpdatedAt, arg.Url)
	return err
}
